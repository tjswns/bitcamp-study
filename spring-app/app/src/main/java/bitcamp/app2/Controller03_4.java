// 세션 다루기 - @SessionAttributes를 적용할 때
...
public class Controller03_4 {
  ...
  public String step0(HttpSession session) {
    ...
    // 없다면 새로 만들어 넘겨 줄 것이다.
    // 어찌 되었든 이 요청 핸들ㄹ러를 실행하는 순간 HttpSessoion 객체를 존재
    ...
    // 예제를 실행하는ㄴ 과정에서 HttpSession 객체를ㄹ 미리 준비할 필요가 있기 때문에
    // 예제 테스트를 위해 이 메서드를 만든 것이다.
    // 이 메서드를 먼저 실행하여 HttpSession 객체를 준비한 후에 Step1234를 테스트하라
    return "세션을 준비했음!";
  }
  ...
  public String step4(
    ...
    // 이 페이지 컨트롤러가 작업을 하는동안 세션에 임시 보관했던 값들은
