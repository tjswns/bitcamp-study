...
public class Controller03_2 {
  ...
  public String handler4(
          // 현재 페이지 컨트롤러의 @SessionSttributes에 지정되지 않은 값 꺼내기
      // => 꺼내지 못한다. name과 age에는 빈 문자열이 저장된다.
      @ModelAttribute("name") String name,
      @ModelAttribute("age") String age,
          // 현재 페이지 컨트롤러의 @SessionSttributes에 지정된 값 꺼내기
        // => 꺼낸다.
      @ModelAttribute("name2") String name2,
      @ModelAttribute("age2") String age2,
      @ModelAttribute("tel2") String tel2) {
    ...
    //@ModelAttribute
    // 1. @SessionAtrribute에 지정된 이름인 경우
    // 1) HttpSession 객체에 값이 있다면 꺼내넘긴다.
    //      없다면, 예외발생!
    // 2. @SessionnAttribute에 지정된 이름이 아닌경우,
    // 1) @SessionAttributes에 지정된 이름이라면, HttpSession 객체에서 꺼내 넘긴다.
    // 2) 없으면 요청파라미터 값을 꺼내 넘긴다.
    // 3) 없으면 빈 문자열을 넘긴다.
