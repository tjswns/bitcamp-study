...
import bitcamp.myapp.vo.Styliing;
...
public class StylingHandler implements Handler {
  ...
  private Styliing[] boards = new Styliing[MAX_SIZE];
  ...
  private void inputStyle(Styliing board) {
    loop: while (true) {
      String menuNo =
  ...
  private void inputBrand(Styliing board) {
    loop: while (true) {
  ...
  private void inputFit(Styliing board) {
    loop: while (true) {
      String menuNo = this.prompt.inputString(
          "좋아하는 브랜드:\n" + "  1. 오버핏\n" + "  2. 슬림핏\n" + "  3. 스탠다드핏\n" + "  4. 그 외\n" + "> ");
