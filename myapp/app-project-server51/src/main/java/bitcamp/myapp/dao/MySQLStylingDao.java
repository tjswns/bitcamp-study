...
public class MySQLStylingDao implements StylingDao {
  ...
  public MySQLStylingDao(DataSource ds, int category) {
    this.ds = ds;
  ...
  public int update(Styling styling) {
    try (PreparedStatement stmt =
        ds.getConnection(false).prepareStatement("update myapp_styling set" + " style=?,"
            + " brand=?" + " 'fit'=?" + " where category=? and styling_no=? and writer=?")) {
      ...
      stmt.setInt(6, styling.getWriter().getNo());
  ...
  public int delete(Styling styling) {
    try (PreparedStatement stmt = ds.getConnection(false).prepareStatement(
        "delete from myapp_styling where category=? and styling_no=? and  writer=?")) {

      stmt.setInt(1, this.category);
      stmt.setInt(2, styling.getNo());
      stmt.setInt(3, styling.getWriter().getNo());
