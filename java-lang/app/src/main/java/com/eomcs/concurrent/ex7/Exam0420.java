...
public class Exam0420 {
  ...
  public static void main(String[] args) {
    ...
    // => shutdown()과 차이점:
    // - 만약 Running 상태의 스레드가 잠시 Not Runnable 상태(sleep()/wait())에 놓인다면,
    // 바로 스레드를 멈출 기회라고 보고 스레드 종료를 시도할 것이다.
    // -즉 실행중인 작업만 완료시키고, 대기 중인 작업은 취소하는 효과가 있다.
    // - 이런 이유로 shutdown() 보다는 스레드 종료를 좀 더 강제하는 효과가 있다.
    // => Running 상태일 때는 작업이 끝나거나 Not Runnable
    ...
    // - shutdown()
    // 진행 중인 작업을 완료하고 대기 중인 작업은 취소하고 그 목록을 리턴한다.
    // - shutdownNow()
